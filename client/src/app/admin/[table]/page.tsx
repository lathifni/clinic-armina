...
import { confirmDeleteAlert, showCreateAlert, showDeleteAlert, showEditeAlert, showErrorAlert } from "@/utils/AlertUtils";
          ...
          Object.keys(data[0]).map((field) => (
            <div key={field} className="mb-4">
              ...
              <input
                ...
                value={field !== "image" ? formik.values[field] || "" : undefined}
              ...
              {data?.map((row: TableRow, index: number) => (
                <tr key={index} className="border-b">
                  {Object.keys(row).map((key) => (
                    <td
                      key={key}
                      className="px-4 py-2 border-b text-ellipsis"
                    >
                      {row[key]}
                    </td>
                  ))}
                  <td className="px-2 flex gap-2 justify-center items-center py-2">
                    <button
                      className="text-blue-500 border-2 rounded p-2 border-blue-500 flex items-center hover:bg-blue-500 hover:text-white"
                    ...
                    <button
                      className="text-red-500 border-2 rounded p-2 border-red-500 flex items-center hover:bg-red-500 hover:text-white"
